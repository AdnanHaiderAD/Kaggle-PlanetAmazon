...
def evaluate_f2(softmax_logits, sigmoid_logits, labels_pl):
  predictions = get_map_predictions(softmax_logits, sigmoid_logits)
  predictions_1ofK = concat_atmos_others(predictions[0], predictions[1])
  
  # compute true positives
  tp = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(predictions_1ofK, tf.constant(1, shape=predictions_1ofK.shape)), tf.equal(labels_pl, tf.constant(1, shape=labels_pl.shape))), tf.float32), 0)
  # compute false positives
  fp = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(predictions_1ofK, tf.constant(1, shape=predictions_1ofK.shape)), tf.equal(labels_pl, tf.constant(0, shape=labels_pl.shape))), tf.float32), 0)
  # compute false negatives
  fn = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(predictions_1ofK, tf.constant(0, shape=predictions_1ofK.shape)), tf.equal(labels_pl, tf.constant(1, shape=labels_pl.shape))), tf.float32))
...
def evaluate_cer(softmax_logits, sigmoid_logits, atmos_pl, other_labels_pl):
  predictions = get_map_predictions(softmax_logits, sigmoid_logits)
  other_labels_accuracy = tf.reduce_mean(tf.cast(tf.equal(predictions[1], other_labels_pl), tf.float32), 0)
  atmos_accuracy = tf.reduce_mean(tf.cast(tf.equal(predictions[0], atmos_pl), tf.float32))
...
def argmax_to_1ofK(x, num_classes):
  return tf.one_hot(x, num_classes)

def concat_atmos_others(atmos_argmax, other_labels_predictions):
  atmos_1ofK = argmax_to_1ofK(atmox_argmax, 4)
  return tf.concat([atmos_1ofK, other_labels_predictions], 1)
